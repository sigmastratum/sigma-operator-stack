...
def main():
    ...
    archive=subprocess.run(["git","-C",os.fspath(repository),"archive","--format=tar",candidate,"--","installers/windows-installer","tools/windows_pe_manifest.py"],check=True,stdout=subprocess.PIPE).stdout
    with tempfile.TemporaryDirectory(prefix="sos-windows-installer-source-") as temporary:
        ...
        manifest=source_root/"installers"/"windows-installer"/"application.manifest"
        resource=source_root/"installers"/"windows-installer"/"rsrc_windows_amd64.syso"
        manifest_tool=source_root/"tools"/"windows_pe_manifest.py"
        subprocess.run([sys.executable,os.fspath(manifest_tool),"build-resource","--manifest",os.fspath(manifest),"--output",os.fspath(resource)],check=True,stdin=subprocess.DEVNULL,stdout=subprocess.PIPE,text=True)
        ...
        verification=subprocess.run([sys.executable,os.fspath(manifest_tool),"verify-pe","--manifest",os.fspath(manifest),"--pe",os.fspath(output)],check=True,stdin=subprocess.DEVNULL,stdout=subprocess.PIPE,text=True).stdout
    ...
    manifest_report=json.loads(verification)
    print(json.dumps({"candidate":candidate,"filename":output.name,"go_version":GO_VERSION,"manifest_sha256":manifest_report["manifest_sha256"],"requested_execution_level":manifest_report["requested_execution_level"],"sha256":digest,"source_tree":tree,"status":"passed","ui_access":manifest_report["ui_access"]},sort_keys=True,separators=(",",":"))); return 0
