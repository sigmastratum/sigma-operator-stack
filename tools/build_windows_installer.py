...
def main():
    ...
    subprocess.run([os.fspath(args.go),"build","-buildvcs=false","-trimpath","-ldflags",f"-buildid= -s -w -X main.candidate={candidate}","-o",os.fspath(output),"."],cwd=repository/"installers"/"windows-installer",env=environment,check=True,stdin=subprocess.DEVNULL)
    ...
    print(json.dumps({"candidate":candidate,"filename":output.name,"go_version":GO_VERSION,"sha256":digest,"status":"passed"},sort_keys=True,separators=(",",":"))); return 0
